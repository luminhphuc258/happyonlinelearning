...
document.addEventListener("DOMContentLoaded", function () {
  // set course name 
  let coursename = localStorage.getItem("courseQuizeName");
  $("#courseQuizeName").text(coursename);
  ...
  accordions.forEach(button => {
    button.addEventListener("click", function () {
      const content = this.nextElementSibling; // Get the corresponding content
      const icon = this.querySelector("svg"); // Get the SVG icon

      // Toggle active class for smooth expansion/collapse
      if (content.style.maxHeight) {
        content.style.maxHeight = null;
        icon.classList.remove("rotate-180"); // Optional: Rotate icon back
      } else {
